...
export async function POST(request: Request) {
  try {
    ...
    const { orderId, paymentId, signature, planId, billingPeriod } = body as {
      orderId: string;
      paymentId: string;
      signature: string;
      planId: string;
    ...
    if (ACTIVE_PROVIDER === "razorpay") {
      if (!orderId || !paymentId || !signature || !planId || !billingPeriod) {
      ...
      // Future Stripe/PayPal verification callbacks
      return NextResponse.json({ success: true });
    }
  } catch (error: any) {
